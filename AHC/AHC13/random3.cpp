...
double calc_score(int N, vector<string> field, const Result &res, double x)
{
    for (auto r : res.move) {
    ...
    return score * x - cnt_parent.size() * (1 - x);
}
...
int main()
{
    ...
    while(1) {
        ...
        double sampling_best_score = -1e9;
        Solver best_s(N, K, field);
        auto best_ret = best_s.solve();
        for (int i = 0; i < sampling; i++){
            Solver s(N, K, field, rand());
            auto ret = s.solve();
            double sampling_score = calc_score(N, field, ret, spend_time / 3.0);
        ...
        spend_time = (end.tv_sec + end.tv_usec / 1e6) - (start.tv_sec + start.tv_usec / 1e6);
        interval = (end.tv_sec + end.tv_usec / 1e6) - (mid.tv_sec + mid.tv_usec / 1e6);
